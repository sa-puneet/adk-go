...
**Parent**: [tool](../AGENTS.md) | **Repository**: An open-source Go toolkit for building,

> Implements an `agentTool` wrapper, enabling the composition of agents by allowing one agent to invoke another as a tool within larger language model workflows. Defines a `Config` structure for tool-specific options and provides a constructor for creating `agentTool` instances.
